...
describe('scKeylevelSweep.evaluate — informational factor telemetry (2026-09-25: no longer a gate, see shared.js)', () => {
  it('legacyPassed is true when every factor is true', () => {
    const result = evaluate(fullPassPayload);
    expect(result.legacyPassed).toBe(true);
  ...
  it.each([
    ...
    'min_rr_ok',
    'not_invalidated',
  ])('legacyPassed is false when %s alone is false, listing it in failed', (factorName) => {
    const result = evaluate({ ...fullPassPayload, [factorName]: false });
    expect(result.legacyPassed).toBe(false);
    expect(result.failed).toEqual([factorName]);
  });

  it('direction_present is matched for "long"/"short" (case-insensitive) and failed otherwise', () => {
    expect(evaluate({ ...fullPassPayload, direction: 'LONG' }).legacyPassed).toBe(true);
    expect(evaluate({ ...fullPassPayload, direction: 'short' }).legacyPassed).toBe(true);
  ...
  it('never throws on a missing/malformed payload — a throwing check just fails that factor', () => {
    ...
    expect(result.legacyPassed).toBe(false);
    expect(result.failed.length + result.missing.length).toBeGreaterThan(0);
  });
});
